...
public class PeoplsPopulationActivity : IActivity
{
    ...
    private DistributionValueContainer probabilityDistributionMaleDeath;
    ...
    {
        this.probabilityDistributionMaleDeath = context.storage.Fetch<DistributionValueContainer>(GlobalBaseKey.probabilityDistributionMaleDeath);
    }
    ...
    public void Loop(GameContext context, int step, ActivityStopHandler stopHandler)
    {
        ...
        context.PeoplsForeach((peopleCore) =>
        {
            var people = new PeopleBaseProxy(peopleCore);
            if( random.NextDouble() <= this.probabilityDistributionMaleDeath.fetchValue(people.year.value))
            {
                context.Kill(peopleCore);
            }
            else
            {
                people.year.value += context.stepLength;
            }
        });
    }
    ...
    public void WillStart(GameContext context)
    {
        ...
        this.probabilityDistributionMaleDeath.setValues(1, array);

        var random = new Random();
        for (int i = 0; i < 15000; i++)
        {
            var people = new PeopleBaseProxy(context.AddPeople());
            people.year.value = random.NextDouble() * 100;
        }
    }
}